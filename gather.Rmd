...
# Even though the plot above showed strong correlation only between death rate
# and SO2, let's include all 6 pollutants for fit 1. Also, let's control for
# population and area since intuitively they are related to the death rate.

fit_1 <- stan_glm(data = covid_aq_no_zero,
         formula = death_rate ~ avg_co + avg_no2 + avg_o3 + avg_pm10 + 
           avg_pm2.5 + avg_so2 + total_pop + total_area, 
         seed = 13,
         refresh = 0)
...
table_1 <- tbl_regression(fit_1, 
  ...
  # Since the coefficients are relatively small, let's use digits = 4.
  
  as_gt() %>% 
  cols_label(estimate = md("**Parameter**")) %>%
  tab_header(title = md("**COVID-19 Death Rate**"),
             subtitle = "How Air Pollutants, Population, and Land Area Predict COVID-19 Death Rates") %>% 
...
# Let's comment out gtsave so that we don't accidently overwrite it when running
# the code chunks.

## table_1 %>%
##  gtsave("table_1.html", path = "final-project/")
